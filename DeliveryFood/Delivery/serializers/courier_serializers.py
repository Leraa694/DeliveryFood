...
class CourierSerializer(serializers.ModelSerializer):
    ...
    def validate_vehicle_type(self, value):
        ...
        if value not in allowed_types:
            raise serializers.ValidationError(f"Тип транспорта должен быть одним из: {', '.join(allowed_types)}.")
        return value
...
class DeliverySerializer(serializers.ModelSerializer):
    ...
    def validate_delivery_status(self, value):
        ...
        if value not in allowed_statuses:
            raise serializers.ValidationError(f"Статус доставки должен быть одним из: {', '.join(allowed_statuses)}.")
        return value
