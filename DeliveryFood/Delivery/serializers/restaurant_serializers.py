...
class RestaurantSerializer(serializers.ModelSerializer):
    ...
    def validate_phone(self, value):
        if len(value) < 10 or len(value) > 15:
            raise serializers.ValidationError("Номер телефона должен содержать от 10 до 15 символов.")
        return value
...
class MenuItemSerializer(serializers.ModelSerializer):
    ...
    def validate_is_available(self, value):
        if not isinstance(value, bool):
            raise serializers.ValidationError("Поле 'Доступно ли блюдо' должно быть булевым значением.")
        return value