...
class RestaurantViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('cuisine_type', openapi.IN_QUERY, type=openapi.TYPE_STRING, description='Тип кухни'),
        ],
    )
    ...
    def restaurants_by_cuisine_type(self, request):
        ...
        if not cuisine_type:
            return Response({"error": "Параметр 'cuisine_type' обязателен."}, status=status_code.HTTP_400_BAD_REQUEST)
...
class MenuItemViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('restaurant_id', openapi.IN_QUERY, type=openapi.TYPE_INTEGER, description='ID ресторана'),
        ],
    )
    ...
    def menu_items_by_restaurant(self, request):
        ...
        if not restaurant_id:
            return Response({"error": "Параметр 'restaurant_id' обязателен."}, status=status_code.HTTP_400_BAD_REQUEST)
